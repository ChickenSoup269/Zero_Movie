...
export class CinemaService {
  ...
  static async getShowtimesByCinemaId(id: string, date?: string, movieId?: string): Promise<any> {
    if (!mongoose.Types.ObjectId.isValid(id)) {
      throw new Error('ID rạp không hợp lệ');
    }
    ...
    const formattedShowtimes = await Promise.all(showtimes.map(async showtime => {
      ...
      const movie = await mongoose.model('Movie').findOne({ tmdbId: showtime.movieId });
      return {
  ...
  // UPDATE
  static async updateCinema(id: string, name?: string, address?: string): Promise<ICinema | null> {
    if (!mongoose.Types.ObjectId.isValid(id)) {
      throw new Error('ID rạp không hợp lệ');
    }
    if (!name && !address) {

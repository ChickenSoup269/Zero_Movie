...
export class ShowtimeSeatService {
  ...
  static async initializeSeats(showtimeId: string, roomId: string): Promise<void> {
    if (!mongoose.Types.ObjectId.isValid(showtimeId) || !mongoose.Types.ObjectId.isValid(roomId)) {
  ...
  // [GET] seatbyshowtime
  static async getSeatsByShowtime(showtimeId: string): Promise<IPopulatedShowtimeSeat[]> {
  ...
  static async cleanUpExpiredShowtimes(): Promise<void> {
    ...
    for (const showtime of expiredShowtimes) {
      const deletedCount = await ShowtimeSeat.deleteMany({ showtimeId: showtime._id.toString() });
      console.log(`Đã xóa ${deletedCount.deletedCount} ShowtimeSeat cho showtime ${showtime._id}`);
    }
  }
  // UPDATE
  static async updateSeatStatus(showtimeId: string, seatId: string, status: 'available' | 'booked' | 'reserved'): Promise<IShowtimeSeat | null> {
    if (!mongoose.Types.ObjectId.isValid(showtimeId) || !mongoose.Types.ObjectId.isValid(seatId)) {

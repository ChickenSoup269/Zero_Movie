...
export class CommentController {
  ...
  static async addComment(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!movieId || !content) {
        res.status(400).json({ message: 'Thiếu movieId hoặc nội dung bình luận' });
        return;
      }
      ...
      res.status(400).json({ message: (error as Error).message || 'Lỗi khi thêm bình luận' });
    }
  }
  ...
  static async getComments(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(400).json({ message: (error as Error).message || 'Lỗi khi lấy danh sách bình luận' });
    }
  }
  ...
  static async updateComment(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(400).json({ message: (error as Error).message || 'Lỗi khi sửa bình luận' });
    }
  }
  ...
  static async deleteComment(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(400).json({ message: (error as Error).message || 'Lỗi khi xóa bình luận' });
    }
  }
}
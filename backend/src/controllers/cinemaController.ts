...
export const getAllCinemas = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: 'Lỗi khi lấy danh sách rạp', error: (error as Error).message });
  }
};
...
export const getShowtimesByCinemaId = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await CinemaService.getShowtimesByCinemaId(id, date as string, movieId as string);
    res.status(200).json({
